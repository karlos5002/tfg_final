...
/**
 * ============================================================================
 * COOPERATIVA SAN JUAN BAUTISTA — API Climática (clima.php)
 * ============================================================================
 ...
 * AGRARIA calculada en servidor según las reglas reales del sector olivarero.
 *
 * Por qué proxy en lugar de fetch directo desde el navegador:
 *   - Permite añadir lógica de negocio (recomendación) sin enviarla al cliente.
 *   - Cachea la respuesta 30 min en sesión: ahorramos llamadas a Open-Meteo
 *     aunque 100 socios entren al panel a la vez.
 *   - Mantiene las coordenadas privadas (no las recibe el navegador).
 *   - Si en el futuro queremos cambiar de Open-Meteo a AEMET, sólo se toca
 *     este archivo: el frontend no se entera.
 *
 * Open-Meteo NO requiere API key — perfecto para una demo / TFG porque
 * funciona out-of-the-box. La docs está en https://open-meteo.com/en/docs.
 * ============================================================================
 */
...
// ─── Solo socios y admins (admin para QA) ────────────────────────────────
if (!isset($_SESSION['rol']) || !in_array($_SESSION['rol'], ['socio', 'admin'], true)) {
    http_response_code(403);
    echo json_encode(['error' => true, 'mensaje' => 'Acceso restringido.']);
    exit;
}
...
// ─── Caché en sesión 30 min: ahorra calls y mejora respuesta ────────────
$cache = $_SESSION['clima_cache'] ?? null;
...
// ─── Llamada al API externo (cURL, no file_get_contents — más fiable) ────
...
curl_setopt_array($ch, [
    ...
    CURLOPT_TIMEOUT        => 8,
    CURLOPT_USERAGENT      => 'Cooperativa-SJB-TFG/1.0',
...
// ─── Reorganizar la respuesta en algo cómodo para el frontend ────────────
$d        = $apiResp['daily'];
$dias     = [];
...
$_SESSION['clima_cache'] = ['ts' => time(), 'data' => $resultado];
 ...
 * Reglas extraídas de las prácticas del sector olivarero extremeño:
 ...
 *   - Helada (<2°C): la aceituna se daña y baja el rendimiento graso.
...
function calcularRecomendacionRecoleccion(array $dias): array
{
    ...
    // 🚫 Lluvia significativa hoy o mañana — NO recolectar
    ...
    // ⚠️ Probabilidad alta de chubascos
    ...
    return [
        ...
        'mensaje' => 'Sin lluvia, sin viento fuerte y temperaturas estables. Condiciones óptimas para recolectar y transportar la aceituna a la almazara.',
    ];
}

/**
 * Mapeo de códigos WMO a emoji.
 * Tabla oficial: https://www.nodc.noaa.gov/archive/arc0021/0002199/1.1/data/0-data/HTML/WMO-CODE/WMO4677.HTM
 */
